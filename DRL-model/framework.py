...
from message_pass import sum_efeat


class ProxPolicyOptimFramework(object):

    def __init__(self, actor_critic, init_lr, clip_value, optim_num_samples, optim_batch_size, critic_loss_coef, reg_coef,
                 max_grad_norm, device):
        self.actor_critic = actor_critic
        self.optimizer = Adam(actor_critic.parameters(), lr=init_lr)
        # self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=4000, gamma=0.1)
    ...
    def update(self, rollout):
        ...
        cnt = 0
        data_loader = rollout.build_update_sampler(self.optim_batch_size, self.optim_num_samples)

        for samples in data_loader:
            (g, obs, actions, old_action_log_probs, old_value_preds, rets, old_advantages) = samples

            obs = obs.to(self.device)
            edge_masks = (obs.select(2, 0).long() == 2).to(self.device)
            ...
            (action_log_probs, entropy, value_preds,
             edge_masks) = self.actor_critic.evaluate_batch(obs.permute(1, 0, 2), g, actions.permute(1, 0))
            action_log_probs = action_log_probs.permute(1, 0)
            value_preds = value_preds.permute(1, 0)
            edge_masks = edge_masks.permute(1, 0)

            diff = action_log_probs - old_action_log_probs
            clamped_diff = torch.clamp(diff, math.log(1.0 - self.clip_value), math.log(1.0 + self.clip_value))
            stacked_diff = torch.stack([diff, clamped_diff], dim=2)
            g = g.to(self.device)
            h = sum_efeat(g, stacked_diff.permute(1, 0, 2)).permute(1, 0, 2)
            ...
            loss = (actor_loss + self.critic_loss_coef * critic_loss + self.reg_coef * reg_loss)
